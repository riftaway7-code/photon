...
// Find all ph-* buckets
console.log('Finding existing ph-* buckets...');
const { Buckets } = await client.send(new ListBucketsCommand({}));
const buckets = Buckets.filter(b => b.Name.startsWith('ph-')).map(b => b.Name);

if (buckets.length === 0) {
  console.log('No ph-* buckets found. Run deploy-s3.js first.');
  process.exit(0);
}
console.log(`Found ${buckets.length} bucket(s): ${buckets.join(', ')}`);
...
console.log(`\nUploading ${files.length} files to ${buckets.length} bucket(s) in parallel...`);
